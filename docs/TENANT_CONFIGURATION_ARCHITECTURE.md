...
### 3.5 Referral — **migrata (132 applicata; 133 di pulizia pronta, da applicare dopo il deploy)**
...
- **Colonne legacy:** la 132 le teneva allineate con due trigger. La **133** (distruttiva, rollback nei commenti) le rimuove insieme ai trigger e alla funzione `referral_config_from_tenant`. Prima verifica che ogni tenant abbia una riga identica alle sette colonne, altrimenti si ferma senza eliminare nulla. Non tocca `customers.referral_*`, `referral_codes` e le catene.
- **Letture:** `getReferralSettings(db, tenantId)` in `lib/loyalty/referralConfig.ts`. Restituisce un oggetto con le **stesse chiavi delle ex colonne**, così la logica esistente non cambia. Precedenza: riga valida e attiva → riga; riga assente (tenant mai configurato dopo la 133) → valori predefiniti della 040, gli stessi che un nuovo tenant aveva con le colonne; disattivata, invalida o illeggibile → `null` (fail closed). Nessun ripiego su `tenants`. Lettori: `processOrderPointsOnDelivery` (catena e anti-frode), `registerWithReferral`, `checkReferralAccessUnlock`, API `/api/loyalty/referrals/{eligibility,generate-code,tree}`, `/compte`, `/compte/parrainage`, `/admin/loyalty`. Le superfici cliente usano `REFERRAL_UNAVAILABLE_VIEW` (`ADMIN_GRANTED_ONLY`, nessun bonus) quando il programma non è disponibile.
- **Scritture:** `LoyaltyConfigSection` → `PATCH /api/admin/loyalty/referral` (`tenant_settings.manage`, invariato). La route rifiuta `SPENDING_THRESHOLD` con soglia ≤ 0, regola documentata dalla 040 ma prima non applicata. Il primo salvataggio di un tenant senza riga la crea attiva; altrimenti l'attivazione esistente resta invariata. `/api/admin/tenant` non accetta campi referral. Il bonus d'iscrizione non è esposto in UI.
- **Esposizione:** S. La 132 revoca il grant pubblico 076 sui sette campi, soglie anti-frode incluse.
- **Ordine di rilascio della 133:** prima il deploy del codice senza ripiego legacy, poi la migrazione.
...
| 3 | Referral — **fatto (132 applicata; 133 di rimozione colonne da applicare dopo il deploy)** | Dipende dalla loyalty; soglie anti-frode rese private dalla 132 | Medio-alto |
