...
- Preparing/new paid orders are `today`, become `urgent` after default 24 h. Managed shipments with a known provider reference and no new tracking event beyond the configurable 72 h threshold are monitored; shipments past the provider-estimated delivery date are urgent but not automatically deemed lost. Ready-for-pickup orders are `monitor`, become `urgent` after 48 h of last `orders.updated_at` (an approximation, not an exact ready timestamp). Verified external pending payments are NOT assumed; unresolved payment sessions are actionable after default 48 h. Assisted drafts older than 24 h enter monitoring.
