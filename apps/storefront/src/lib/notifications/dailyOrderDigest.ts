...
export function renderDigestHtml(name:string,date:string,items:DigestItem[],
  ...
  const section=(p:Priority,label:string,color:string)=>{
    ...
    return '<h2 style="color:'+color+'">'+escape(label)+' · '+group.length+'</h2>'+
      group.slice(0,5).map(i=>'<div style="border:1px solid '+(p==='urgent'?'#fecaca':'#e2e8f0')+
        ';background-color:'+(p==='urgent'?'#fff7f7':'#ffffff')+
        ';padding:15px;border-radius:10px;margin:9px 0">'+
        (p==='urgent'?'<p style="margin:0 0 10px;color:#991b1b;font-weight:bold">URGENT</p>':'')+
        '<strong>'+escape(i.reference+' · '+i.customer)+'</strong><p>'+escape(i.reason)+'</p>'+
        '<p style="color:#475569">'+escape(i.action)+'</p>'+
        (p==='urgent'
          ?'<a href="'+escape(i.url)+'" style="display:block;box-sizing:border-box;background-color:#b91c1c;color:#ffffff;text-decoration:none;text-align:center;font-weight:bold;font-size:16px;line-height:24px;padding:13px 16px;border-radius:8px">Traiter cette commande →</a>'
          :'<a href="'+escape(i.url)+'" style="color:'+(p==='today'?'#1d4ed8':'#475569')+'">Voir la fiche →</a>')+
        '</div>').join('')+
