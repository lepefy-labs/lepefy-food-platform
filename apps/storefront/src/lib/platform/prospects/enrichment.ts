...
export async function enrichProspect(p:Prospect,deps:Dependencies={free:osmProvider,fallback:googleProvider,website:enrichWebsite},freeEnabled=true) {
  const metrics={osm_matches:0,osm_ambiguous:0,website_discovered:0,website_crawled:0,
    website_blocked:0,google_fallback_used:0,google_quota_skipped:0,complete:0,partial:0,failed:0};
  ...
  if (freeEnabled && !manualNoWebsite && (!combined.website_url || !combined.phone)) {
    try {
      ...
      if(result.patch.osm_metadata?.matched)metrics.osm_matches++;
      if(result.patch.osm_metadata?.ambiguous)metrics.osm_ambiguous++;
  ...
  if (!combined.website_url && !manualNoWebsite) {
    ...
    } catch { /* Optional fallback must not discard free-source evidence. */ }
  }
