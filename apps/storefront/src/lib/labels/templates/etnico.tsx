...
// Ordine piatto voluto dal design (grid 2 colonne, auto-flow di riga):
// Energia | Carboidrati / di cui grassi saturi | di cui zuccheri (sub) / Grassi | Fibre / Proteine | Sale.
// Un array annidato (righe principali + sotto-righe in coda) rompe questo abbinamento colonna per colonna,
// quindi qui l'ordine dei children riflette esattamente la sequenza visiva richiesta.
const NUTRITION_FLAT_ROWS: Array<{ key: NutritionKey; label: string; sub?: boolean }> = [
  { key: 'carbs_g', label: 'Carboidrati' },
  { key: 'saturated_fat_g', label: 'di cui grassi saturi', sub: true },
  { key: 'sugars_g', label: 'di cui zuccheri', sub: true },
 ...
 * Layout derivato dal mock 100mm×75mm del design handoff, ma le rotture strutturali
 * principali (altezza foto, inizio pannello dati, inizio titolo) sono espresse come frazione di
...
export function EtnicoLabelTemplate({
  ...
  // Foto ridotta rispetto al mock originale (44mm/75mm) per dare al pannello dati
  // (nutrizione + footer legale) più respiro con dati reali lunghi — vedi BUG1/BUG4.
  const photoHeightMm = labelHeightMm * (36 / 75);
  const titleTopMm = labelHeightMm * (27 / 75);
  ...
  return (
    <div style={{
      ...
      {/* Badge "100% Naturale" ruotato — effetto sticker come nel design */}
      {naturalBadge && (
        <div style={{
      ...
      {/* Pillola bandiera + paese d'origine — stesso campo (product.country_of_origin) usato dagli altri 3 template */}
      {showOrigin && (
        <div style={{
      ...
      {/* Blocco titolo: nome FR grande + sottotitolo IT/EN (stesso campo product.name_alt degli altri template, nessun uppercase forzato) */}
      ...
      {/* Pannello dati sotto la foto — altezza fissa (top:photoHeightMm, bottom:0); il contenuto
          è avvolto in un wrapper "shrink-to-fit" (stesso meccanismo già usato da fullbleed.tsx)
          che lo scala fino a un minimo del 75% se necessario, così il footer legale (importatore +
          distributore, dato legale obbligatorio) non viene mai tagliato dal overflow:hidden esterno,
          qualunque sia la lunghezza reale di indirizzi/ingredienti. */}
      <div style={{
        position: 'absolute', top: `${photoHeightMm}mm`, left: 0, right: 0, bottom: 0,
        padding: '1.8mm 3mm', overflow: 'hidden',
      }}>
        <div
          id={`label-etnico-panel-${product.id}`}
          className="label-shrink-to-fit"
          style={{ display: 'grid', gridTemplateRows: 'auto auto 1fr auto', minHeight: '100%', width: '100%', transformOrigin: 'top left' }}
        >
          <div style={{
          ...
          {sections.nutrition && product.nutrition ? (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '2.5mm', rowGap: '0.4mm', fontSize: '2mm', color: TEXT, marginTop: '0.5mm' }}>
                ...
                {NUTRITION_FLAT_ROWS.filter((r) => product.nutrition?.[r.key] != null).map((r) => (
                  <div key={r.key} style={{ display: 'flex', justifyContent: 'space-between', color: r.sub ? SUB_TEXT : TEXT }}>
                    <span>{r.label}</span>
                    {r.sub
                      ? <span>{product.nutrition?.[r.key]} g</span>
                      : <strong>{product.nutrition?.[r.key]} g</strong>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ fontSize: '1.85mm', color: INGREDIENTS_TEXT, lineHeight: 1.4, marginTop: '0.6mm' }}>
            ...
            {sections.allergens && product.allergens_text && (
              <div style={{ marginTop: '0.3mm' }}><strong style={{ color: TEXT }}>Allergeni:</strong> {product.allergens_text}</div>
            )}
            {sections.usage && product.usage_instructions && (
              <div style={{ marginTop: '0.3mm' }}><strong style={{ color: TEXT }}>Consigli d&apos;uso:</strong> {product.usage_instructions}</div>
            )}
          </div>

          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end',
            borderTop: `0.2mm solid ${RULE}`, paddingTop: '1mm', marginTop: '0.8mm', gap: '2mm',
          }}>
            <div style={{ flex: 1, minWidth: 0, fontSize: '1.6mm', color: LEGAL_TEXT, lineHeight: 1.4 }}>
            ...
            <div style={{ display: 'flex', gap: '1.5mm', alignItems: 'flex-end', flexShrink: 0 }}>
              ...
              {/* dark=000000: forza moduli scuri neri indipendentemente dal primary_color del tenant,
                  per garantire sempre contrasto/scansionabilità sul look fisso di questo template. */}
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={`${process.env.NEXT_PUBLIC_STOREFRONT_URL}/api/card/qr-code?format=png&size=200&dark=000000`}
                alt=""
      ...
      {/* Shrink-to-fit del pannello dati — stesso meccanismo/script di fullbleed.tsx: Gotenberg usa
          Chromium reale ed esegue il JS prima della cattura PDF. Class-based e idempotente perché
          buildSheetHtml duplica lo stesso markup in ogni cella del foglio. */}
      <script
