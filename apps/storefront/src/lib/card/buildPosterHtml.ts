...
export function buildPosterHtml({ tenant, paymentMethods, qrUrl }: BuildPosterHtmlParams): string {
  const bodyHtml = renderToStaticMarkup(
    PosterTemplate({ tenant, paymentMethods, qrUrl })
  );
...
<style>
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .body { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6mm; padding: 8mm; text-align: center; }
