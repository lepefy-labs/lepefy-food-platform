...
const LEGACY_TENANT_ADMIN_PERMISSIONS = [
  'orders.view','orders.manage','shop_payments.confirm','catalog.view','catalog.manage','customers.view','customers.manage','segments.manage','campaigns.view','campaigns.manage','reviews.view','reviews.moderate','reviews.manage','shipping.view','shipping.manage','loyalty.manage','loyalty.scan','growth.manage','growth.payouts.manage','ai_knowledge.manage',
...
export async function getAdminAccessContext(userId: string, tenantId: string | null): Promise<AdminAccessContext | null> {
  ...
  if (!membershipError && membership) {
    ...
    if (role) return {
      userId: admin.id, email: admin.email, legacyRole: admin.role, tenantId: membership.tenant_id,
      roleId: role.id, roleCode: role.code, roleName: role.name, scope: role.scope === 'platform' ? 'platform' : 'tenant',
      permissions: (rolePermissions ?? []).map((row) => row.permission_key), isPlatformOwner: role.code === 'platform_owner',
      profileCompleted, firstName, lastName, nickname, rbacSource: 'dynamic',
    };
  }
  if (admin.role !== 'platform_owner' && tenantId && admin.tenant_id !== tenantId) return null;
  return {
    userId: admin.id, email: admin.email, legacyRole: admin.role,
    tenantId: admin.role === 'platform_owner' ? null : admin.tenant_id,
    roleId: null, roleCode: admin.role, roleName: admin.role,
    scope: admin.role === 'platform_owner' ? 'platform' : 'tenant', permissions: legacyPermissions(admin.role),
    isPlatformOwner: admin.role === 'platform_owner', profileCompleted, firstName, lastName, nickname, rbacSource: 'legacy',
  };
}
