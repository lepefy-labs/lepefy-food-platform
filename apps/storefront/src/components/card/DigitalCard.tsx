...
import {
  IconMapPin, IconClock, IconShoppingBag, IconUserPlus, IconBrandWhatsapp,
...
const COPY = {
  fr: {
    ...
    payText: 'Plusieurs méthodes sûres et rapides sont disponibles pour régler vos commandes.',
  ...
  it: {
    ...
    payText: 'Sono disponibili diversi metodi sicuri e rapidi per effettuare i tuoi pagamenti.',
...
interface Props {
  tenant: {
    name: string; tagline: string | null; logo_url: string | null; primary_color: string;
    secondary_color: string; accent_light: string; click_collect_address: string | null;
    click_collect_hours: string | null; click_collect_hours_it: string | null;
...
export function DigitalCard({ tenant, socialLinks, paymentMethods }: Props) {
  ...
  const secondaryForeground = readableForeground(tenant.secondary_color);
  ...
  return (
    <div className="min-h-screen bg-gray-50 sm:flex sm:items-start sm:justify-center sm:px-4 sm:py-10">
                    ...
                    {socialLinks.map((link) => {
                      ...
                      return (
                        <a
                          ...
                          className="flex h-11 w-11 items-center justify-center rounded-full shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                          style={{ background: meta.badgeBackground, '--tw-ring-color': tenant.primary_color } as React.CSSProperties}
                        >
      ...
      {!inPayment && (
        <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center border-t border-gray-200/70 bg-white/90 px-4 pt-2 backdrop-blur-md" style={{ paddingBottom: 'calc(.5rem + env(safe-area-inset-bottom))' }}>
          <a
            href="/api/card/vcard"
            download
            className="flex min-h-12 w-full max-w-md items-center justify-center gap-2 rounded-xl text-sm font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
            style={{ backgroundColor: tenant.primary_color, '--tw-ring-color': tenant.primary_color } as React.CSSProperties}
          >
