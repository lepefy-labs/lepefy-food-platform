...
import { StripeExpressWalletStep } from '@/components/payments/StripeExpressWalletStep';
...
const COPY = {
  fr: { title:'Paiement par carte', step1:'Étape 1 sur 2', step2:'Étape 2 sur 2', amount:'Montant à payer', amountPlaceholder:'0,00', name:'Votre nom', namePlaceholder:'Jean Dupont', email:'Votre email', emailPlaceholder:'nom@email.com', continue:'Continuer', processing:'Traitement en cours…', pay:'Payer', secure:'Paiement sécurisé par Stripe', thanks:'Merci !', received:'Paiement reçu.', back:'Retour', returnCard:'Retour à la carte', generic:'Une erreur est survenue. Veuillez réessayer.', invalid:`Le montant doit être compris entre ${MIN_AMOUNT} et ${MAX_AMOUNT} €.`, billing:'Si un pays est demandé ci-dessous, indiquez celui associé à votre carte bancaire (facturation), pas votre position actuelle.', warning:'N’interrompez pas le paiement : après l’initialisation, ne fermez pas et n’actualisez pas cette page jusqu’à la confirmation.',
        applePayTitle:'Apple Pay', wallet:{ secureTitle:'Paiement sécurisé', faceIdHint:'Vous confirmerez avec Face ID ou Touch ID.', notShowingHint:'Apple Pay ne s’affiche pas ? Revenez en arrière et choisissez Carte bancaire.', unavailableTitle:'Apple Pay n’est pas disponible sur cet appareil', unavailableText:'Aucune carte n’est configurée dans Apple Wallet, ou ce navigateur ne permet pas Apple Pay.', useCard:'Payer par carte bancaire', loading:'Chargement d’Apple Pay…' } },
  it: { title:'Pagamento con carta', step1:'Passaggio 1 di 2', step2:'Passaggio 2 di 2', amount:'Importo da pagare', amountPlaceholder:'0,00', name:'Nome', namePlaceholder:'Mario Rossi', email:'Email', emailPlaceholder:'nome@email.com', continue:'Continua', processing:'Elaborazione in corso…', pay:'Paga', secure:'Pagamento sicuro con Stripe', thanks:'Grazie!', received:'Pagamento ricevuto.', back:'Indietro', returnCard:'Torna alla card', generic:'Si è verificato un errore. Riprova.', invalid:`L'importo deve essere compreso tra ${MIN_AMOUNT} e ${MAX_AMOUNT} €.`, billing:'Se viene richiesto un paese qui sotto, indica quello associato alla tua carta (fatturazione), non la tua posizione attuale.', warning:'Non interrompere il pagamento: dopo l’avvio, non chiudere e non aggiornare questa pagina fino alla conferma.',
        applePayTitle:'Apple Pay', wallet:{ secureTitle:'Pagamento sicuro', faceIdHint:'Confermerai con Face ID o Touch ID.', notShowingHint:'Apple Pay non compare? Torna indietro e scegli Carta bancaria.', unavailableTitle:'Apple Pay non è disponibile su questo dispositivo', unavailableText:'Nessuna carta configurata in Apple Wallet, oppure questo browser non supporta Apple Pay.', useCard:'Paga con carta bancaria', loading:'Caricamento di Apple Pay…' } },
} as const;

function isValidEmail(value: string) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.trim()); }

// mode="apple_pay" : même étape 1 (montant, nom, email, même validation),
// étape 2 = bouton Apple Pay seul (StripeExpressWalletStep) au lieu du
// formulaire carte. tenantColor vaut alors le noir Apple Pay (methodColor).
export function CardQuickPay({ tenantColor, currency, lang, onBack, onReturnToCard, mode = 'card', onUseCard }: { tenantColor:string; currency:string; lang:Lang; onBack:()=>void; onReturnToCard:()=>void; mode?:'card'|'apple_pay'; onUseCard?:()=>void }) {
  const t = COPY[lang];
  ...
  return <div>
    ...
    <div className="mb-6"><p className="text-xs font-semibold uppercase tracking-wide" style={{color:tenantColor}}>{step===1?t.step1:t.step2}</p><h2 className="mt-1 text-2xl font-bold text-gray-900" style={{fontFamily:'var(--font-card-heading)'}}>{mode==='apple_pay'?t.applePayTitle:t.title}</h2></div>
    ...
    </div> : <div><div className="mb-5 rounded-2xl border border-gray-100 bg-gray-50 p-4"><p className="text-xs text-gray-500">{t.amount}</p><p className="mt-1 text-2xl font-bold text-gray-900">{formatPrice(confirmedAmount,currency)}</p></div>{mode==='apple_pay'?<StripeExpressWalletStep module="card" amount={confirmedAmount} currency={currency} returnUrl={`${window.location.origin}/card`} locale={lang} labels={t.wallet} createIntent={createIntent} onError={setError} onSucceeded={()=>setPaid(true)} onUseCard={onUseCard}/>:<StripePaymentStep module="card" amount={confirmedAmount} currency={currency} color={tenantColor} returnUrl={`${window.location.origin}/card`} referenceId={null} payLabel={`${t.pay} ${formatPrice(confirmedAmount,currency)}`} processingLabel={t.processing} billingCountryHint={t.billing} paymentWarning={t.warning} locale={lang} createIntent={createIntent} onError={setError} onSucceeded={()=>setPaid(true)}/>}{error&&<p role="alert" className="mt-3 text-sm text-red-600">{error}</p>}<p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-gray-400"><IconLock size={14}/>{t.secure}</p></div>}
  </div>;
}
