...
async function handlePATCH(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .eq('id', tenant.id)
    // Echo only the editable projection: the full row holds server-only
    // values (provider keys, private assistant context, billing data).
    .select(['id', ...EDITABLE_TENANT_FIELDS].join(','))
    .single();
