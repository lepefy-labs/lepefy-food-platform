...
import { buildReservationListHtml, loadEventReservationExportData } from '@/lib/events/adminReservationExports';
...
export async function GET(req: NextRequest) {
  ...
  const pdfBuffer = await htmlToPdf(buildReservationListHtml(data, tenant.name, tenant.logo_url));
  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': 'inline; filename="liste-reservations.pdf"',
