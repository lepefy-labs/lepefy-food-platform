...
const EDITABLE_FIELDS = [
  'title', 'slug', 'description', 'date_start', 'booking_closes_at', 'booking_close_reports_fallback_hours', 'location',
...
function sanitizeHighlights(value: unknown): { data: unknown; error: string | null } {
  ...
  const cleaned = (value as HighlightInput[]).slice(0, MAX_HIGHLIGHTS).map((h) => ({
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  for (const field of EDITABLE_FIELDS) {
    ...
    if (field === 'show_remaining_places') {
      if (typeof body[field] !== 'boolean') return NextResponse.json({ error: 'show_remaining_places doit être un booléen.' }, { status: 400 });
      patch[field] = body[field];
      continue;
    }
    if (field === 'booking_close_reports_fallback_hours') {
      const value = Number(body[field]);
      if (!Number.isInteger(value) || value < 1 || value > 168) {
        return NextResponse.json({ error: 'booking_close_reports_fallback_hours doit être un entier entre 1 et 168.' }, { status: 400 });
      }
      patch[field] = value;
      continue;
    }
    if (field === 'on_site_price_list_image_url') {
      ...
      if (value !== null && typeof value !== 'string') return NextResponse.json({ error: 'on_site_price_list_image_url doit être une URL ou null.' }, { status: 400 });
  ...
  if (body.capacity_total !== undefined) return NextResponse.json({ error: 'La capacité se modifie depuis l’action dédiée « Gérer la capacité ».' }, { status: 400 });
  if (Object.keys(patch).length === 0) return NextResponse.json({ error: 'Aucun champ valide à mettre à jour.' }, { status: 400 });

  const supabase = createServiceClient();
  const { data: current } = await supabase.from('events').select('*').eq('id', params.id).eq('tenant_id', tenant.id).maybeSingle();
  ...
  const { data, error } = await supabase.from('events').update(patch).eq('id', params.id).eq('tenant_id', tenant.id).select('*').single();
  if (error) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (count && count > 0) return NextResponse.json({ error: 'Impossible de supprimer un événement ayant des réservations — annulez-le plutôt.' }, { status: 409 });
