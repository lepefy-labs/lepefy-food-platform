...
interface NewForm { email: string; label: string; notify_card_payment: boolean; notify_order_stock_conflict: boolean; }
function emptyForm(): NewForm { return { email: '', label: '', notify_card_payment: true, notify_order_stock_conflict: false }; }
    ...
    <section className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="mt-4 rounded-xl border border-dashed border-sky-200 bg-sky-50/30 p-3">
          ...
          <div className="mt-3 flex flex-wrap gap-4"><label className="flex items-center gap-2 text-xs text-gray-600"><input type="checkbox" checked={newForm.notify_card_payment} onChange={(e) => setNewForm({ ...newForm, notify_card_payment: e.target.checked })} />Paiement carte</label><label className="flex items-center gap-2 text-xs text-gray-600"><input type="checkbox" checked={newForm.notify_order_stock_conflict} onChange={(e) => setNewForm({ ...newForm, notify_order_stock_conflict: e.target.checked })} />Conflit de stock</label></div>
