...
export function NotificationRecipientsSection({ initialRecipients }: { initialRecipients: TenantNotificationRecipient[] }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <header className="flex items-start gap-3 border-b border-sky-100 bg-sky-50/80 px-4 py-3.5 dark:border-gray-800 dark:bg-gray-900"><div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/80 text-sky-600 shadow-sm dark:bg-gray-800 dark:text-sky-300"><IconBell size={19} stroke={1.7} /></div><div><h2 className="text-sm font-semibold text-sky-700 dark:text-sky-200">Notifications</h2><p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Destinataires et événements internes</p></div></header>
      <div className="p-4 sm:p-5">
        ...
        <div className="space-y-2">
          {recipients.map((r) => <div key={r.id} className="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-950/30">
            <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--admin-primary-soft)] text-xs font-semibold text-[var(--admin-primary-fg)]">{(r.label || r.email).slice(0, 2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-gray-900 dark:text-gray-100">{r.label || r.email}</p><p className="truncate text-xs text-gray-500">{r.email}</p></div><span className={`rounded-full px-2 py-1 text-[11px] font-medium ${r.active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{r.active ? 'Actif' : 'Inactif'}</span></div>
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {toggle(r, 'notify_card_payment', 'Paiement carte')}
