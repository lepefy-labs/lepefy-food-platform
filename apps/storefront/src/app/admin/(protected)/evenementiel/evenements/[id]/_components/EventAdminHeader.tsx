  ...
  return (
    <>
      <Link
        ...
        className="mb-4 inline-flex min-h-11 items-center gap-1.5 text-sm text-gray-500 transition-colors hover:text-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] dark:text-gray-400 dark:hover:text-gray-200"
      >
        ...
        <div className="flex flex-wrap items-start gap-2">
          {publicEnabled ? (
            <Link
              ...
              className="inline-flex min-h-11 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:bg-white/5"
            >
              Voir la page <IconExternalLink size={14} aria-hidden="true" />
            </Link>
          ) : (
            <span className="inline-flex min-h-11 items-center rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm font-medium text-gray-400 dark:border-gray-800 dark:bg-gray-900/60 dark:text-gray-500" title="La page publique n’est pas disponible pour un brouillon ou un événement annulé.">
              Page publique indisponible
            </span>
          )}

          <div className="min-w-44">
            <label htmlFor="event-status" className="sr-only">Statut de l’événement</label>
            <select
              id="event-status"
              value={event.status}
              onChange={(e) => onStatusChange(e.target.value as EventStatus)}
              disabled={savingStatus}
              className="min-h-11 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] disabled:opacity-60 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
            >
      ...
      <div className="mt-5 overflow-x-auto border-b border-gray-200 dark:border-gray-800" role="tablist" aria-label="Sections de l’événement">
        <div className="flex min-w-max gap-1">
          {TABS.map((tab) => (
            <button
              ...
              className={`min-h-11 border-b-2 px-3 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] ${
                activeTab === tab.value
                  ? 'border-[var(--color-primary)] text-[var(--color-primary-dark)] dark:text-white'
