...
import { IconAlertTriangle, IconCheck, IconDotsVertical, IconPencil, IconReceiptRefund, IconSearch, IconSend, IconX } from '@tabler/icons-react';
    ...
    <div className="mt-4 space-y-3">
      <div className="flex flex-col gap-2.5 rounded-xl border border-gray-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900">
        <label className="relative block w-full min-w-0 sm:w-[420px] sm:max-w-[48%]">
          ...
          <input value={search} onChange={(e) => onSearchChange(e.target.value)} placeholder="Rechercher un client ou un email" className="min-h-10 w-full rounded-lg border border-gray-200 bg-white pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" />
        </label>
        <div className="inline-flex w-fit max-w-full gap-0.5 overflow-x-auto rounded-lg bg-gray-50 p-1 dark:bg-gray-950" aria-label="Filtrer les réservations par statut">
          {([
            ['all', 'Toutes'], ['confirmed', 'Confirmées'], ['refunded', 'Remboursées'], ['cancelled', 'Annulées'],
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="refund-dialog-title" aria-describedby="refund-dialog-description" className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl dark:border-gray-700 dark:bg-gray-900">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p id="refund-dialog-description" className="mt-1.5 text-sm leading-5 text-gray-600 dark:text-gray-300">
                  Cette action remboursera <strong className="text-gray-900 dark:text-white">{formatPrice(refundTarget.amount_paid, currency)}</strong> à <strong className="text-gray-900 dark:text-white">{refundTarget.customer_name}</strong> et libérera <strong className="text-gray-900 dark:text-white">{refundTarget.quantity_remaining} place{refundTarget.quantity_remaining > 1 ? 's' : ''}</strong>.
                </p>
