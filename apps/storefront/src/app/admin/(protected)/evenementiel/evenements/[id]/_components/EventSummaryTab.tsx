      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(300px,.8fr)]">
        <div className="space-y-4">
          <section className={cardClass}>
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-800"><div><h2 className="text-sm font-semibold text-gray-950 dark:text-white">Prêt à publier</h2><p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Checklist dérivée des données de l’événement.</p></div><span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">{readinessDone} / {readiness.length}</span></div>
            <div className="divide-y divide-gray-100 dark:divide-gray-800">{readiness.map((item) => <div key={item.label} className="grid grid-cols-[24px_minmax(0,1fr)_auto] items-center gap-2.5 px-4 py-2.5"><span className={`grid h-5 w-5 place-items-center rounded-full ${item.ok ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : item.required ? 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'}`}>{item.ok ? <IconCheck size={12} /> : <IconAlertTriangle size={12} />}</span><div><p className="text-sm font-medium text-gray-900 dark:text-gray-100">{item.label}</p><p className="text-2xs text-gray-400">{item.meta}</p></div>{!item.ok && !item.required && <button type="button" onClick={onOpenPage} className="min-h-9 text-xs font-semibold text-[var(--color-primary-dark)]">Compléter</button>}</div>)}</div>
