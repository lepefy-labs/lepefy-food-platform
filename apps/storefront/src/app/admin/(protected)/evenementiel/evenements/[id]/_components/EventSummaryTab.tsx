...
import { IconCheck, IconClock, IconExternalLink, IconScan, IconAlertTriangle } from '@tabler/icons-react';
    ...
    <div className="mt-5 space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(300px,.8fr)]">
        <div className="space-y-4">
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-800">
              {readiness.map((item) => (
                <div key={item.label} className="grid grid-cols-[24px_minmax(0,1fr)_auto] items-center gap-2.5 px-4 py-2.5">
                  <span className={`grid h-5 w-5 place-items-center rounded-full ${item.ok ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : item.required ? 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'}`}>
                    {item.ok ? <IconCheck size={12} /> : <IconAlertTriangle size={12} />}
                  </span>
          ...
          <section className={cardClass}>
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-800"><div><h2 className="text-sm font-semibold text-gray-950 dark:text-white">Paiements à vérifier</h2><p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Les places ne sont réservées qu’après confirmation.</p></div><span className="text-xs font-semibold text-amber-700 dark:text-amber-300">{pendingRequests.length > 0 ? `${pendingRequests.length} en attente` : 'À jour'}</span></div>
            {pendingRequests.length === 0 ? <div className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400">Aucun paiement à vérifier.</div> : (
              <div className="divide-y divide-gray-100 dark:divide-gray-800">{pendingRequests.map((request) => {
                ...
                return <div key={request.id} className="grid gap-3 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{request.customer_name || request.customer_email} · {request.payment_method_label}</p><p className="mt-0.5 truncate text-xs text-gray-500 dark:text-gray-400">{summary} · {elapsedLabel(request.created_at)}</p></div><div className="flex items-center justify-between gap-3 sm:justify-end"><span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{formatPrice(request.amount, currency)}</span><ConfirmPaymentButton endpoint={`/api/admin/evenementiel/reservation-requests/${request.id}/confirm-payment`} label="Confirmer réception" confirmingLabel="Confirmation…" className="min-h-10 rounded-lg bg-amber-600 px-3 text-xs font-semibold text-white transition-opacity disabled:opacity-50" onSuccess={(warning) => onPendingConfirmed(request.id, warning)} /></div></div>;
              })}</div>
            )}
          </section>
