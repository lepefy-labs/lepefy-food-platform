...
export default async function AdminEvenementielOverviewPage() {
  ...
  const overview = await getEvenementielOverview(tenant.id);
    ...
    <div className="mx-auto w-full max-w-7xl space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <details className="relative self-start">
          <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-lg bg-[var(--color-primary)] px-4 py-2 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus-visible:ring-offset-2 dark:ring-offset-gray-950">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <OverviewMetricCard
          ...
          tone={overview.pendingPaymentsCount > 0 ? 'red' : 'neutral'}
        />
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(300px,.9fr)]">
