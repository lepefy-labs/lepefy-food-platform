...
type Snapshot = {prospects:Prospect[];total:number;page:number;pageSize:number;counts:number[];runs:Run[];google:{enabled:boolean;limit:number;used:number | null;available:boolean}};
export default function ProspectsClient() {
  const [data,setData] = useState<Snapshot | null>(null), [query,setQuery] = useState('collection=unverified'), [selected,setSelected] = useState<string[]>([]);
  ...
  return <div className="mx-auto max-w-7xl space-y-5 text-gray-950 dark:text-gray-100">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">{['Total','À enrichir','Qualifiés','Prioritaires','Démo','Gagnés'].map((label,i) => <div className={card} key={label}><p className="text-xs text-gray-500">{label}</p><p className="mt-1 text-2xl font-semibold">{data?.counts[i] ?? '—'}</p></div>)}</div>
    <p className="text-sm text-gray-500">Découvert → À enrichir → Enrichi → Qualifié → Contacté → Réponse → Démo → Pilote → Gagné</p>
    ...
    <form className={card+' space-y-3'} onSubmit={e => {e.preventDefault();const p = new URLSearchParams();new FormData(e.currentTarget).forEach((v,k) => {if (String(v)) p.set(k,String(v));});setQuery(p.toString());}}>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-sm">Collecte<select name="collection" className={field} defaultValue="unverified"><option value="unverified">À enrichir / actualiser</option><option value="enriched">Évaluation enrichie</option><option value="">Tous</option></select></label>
        <label className="text-sm">Tri<select name="sort" className={field}><option value="collection">Priorité de collecte</option><option value="fit">Fit Score décroissant</option></select></label>
    ...
    <section className={card+' space-y-4'} aria-busy={loading}>
      <div className="flex flex-wrap items-center gap-3"><h2 className="mr-auto font-semibold">{data?.total ?? '—'} prospects</h2>
        ...
        <button className={button} disabled={busy} onClick={() => void enrich(true)}>Enrichir les non vérifiés (max. {CONFIG.enrichmentBatch})</button></div>
      ...
      <div className="hidden overflow-x-auto md:block"><table className="w-full text-left text-sm"><thead><tr className="border-b border-gray-200 text-xs text-gray-500 dark:border-gray-700">
        {['Sélection','Commerce','Catégorie','Localisation','Site','Fit Score','Données','Qualification','Statut','Enrichissement','Actions'].map(c => <th className="px-2 py-3 font-medium" key={c}>{c}</th>)}</tr></thead>
        <tbody>{data?.prospects.map(p => <tr key={p.id} className="border-b border-gray-100 dark:border-gray-800">
          ...
          <td className="px-2 py-3"><Fit p={p} /></td><td className="px-2 py-3"><Completeness p={p} /></td><td className="px-2 py-3"><Qualification p={p} /></td><td className="px-2 py-3"><Badge value={p.status} /></td>
          <td className="px-2 py-3 text-xs">{dateLabel(p.last_enriched_at)}<p><CollectionStatus p={p} /></p></td><td className="px-2 py-3"><Link className={secondary} href={'/admin/platform/prospects/'+p.id}>Voir</Link></td>
        </tr>)}</tbody></table></div>
      <div className="space-y-3 md:hidden">{data?.prospects.map(p => <article className="space-y-2 rounded-xl border border-gray-200 p-3 dark:border-gray-700" key={p.id}>
        ...
        <p className="text-sm">{p.business_category} · {p.city}</p><div className="flex flex-wrap items-center gap-3"><Fit p={p} /><Completeness p={p} /><Qualification p={p} /><Badge value={p.status} /></div>
        <p className="text-xs text-gray-500"><CollectionStatus p={p} /> · {dateLabel(p.last_enriched_at)}</p>{p.do_not_contact && <p className="text-sm text-red-700">Ne pas contacter</p>}
