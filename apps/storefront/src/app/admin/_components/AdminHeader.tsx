...
export default function AdminHeader({ platformName, platformLogoUrl, tenantName, categories, workspace, shopAdminUrl, eventsAdminUrl, isPlatformOwner = false, adminEmail, pendingPaymentsCount = 0, pendingEventRequestsCount = 0, pendingRentalRequestsCount = 0, newInquiriesCount = 0 }: AdminHeaderProps) {
    ...
    <header className="sticky top-0 z-30 flex min-h-[57px] items-center gap-2 border-b border-[var(--admin-border)] bg-white/95 px-3 py-2.5 backdrop-blur dark:border-gray-800 dark:bg-gray-900/95 md:gap-3 md:px-6">
      ...
      <details className="group relative">
        <summary className="flex min-h-10 cursor-pointer list-none items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-[var(--admin-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-primary)]">
          ...
          <div className="min-w-0 text-left"><p className="max-w-32 truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{workspaceLabel}</p><p className="max-w-36 truncate text-[10px] text-gray-400">{tenantName}</p></div><IconChevronDown size={13} className="text-gray-400 transition group-open:rotate-180" />
        </summary>
