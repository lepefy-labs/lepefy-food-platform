...
import { IconShoppingBag, IconChartBar, IconPackage, IconUsers, IconTag, IconSettings, IconCreditCard, IconSparkles, IconGift, IconPhoto, IconStar, IconScan, IconTruck, IconCalendarEvent, IconBell, IconFileInvoice, IconToolsKitchen2, IconBriefcase } from '@tabler/icons-react';
import type { AdminWorkspace } from '@/lib/admin/workspace';

interface AdminSidebarProps { categories: { id: string; name: string; slug: string }[]; workspace?: AdminWorkspace; pendingPaymentsCount?: number; pendingEventRequestsCount?: number; pendingRentalRequestsCount?: number; newInquiriesCount?: number; isPlatformOwner?: boolean; }

export default function AdminSidebar({ workspace = 'shop', pendingPaymentsCount = 0, pendingEventRequestsCount = 0, pendingRentalRequestsCount = 0, newInquiriesCount = 0, isPlatformOwner = false }: AdminSidebarProps) {
  ...
  return <nav className="flex h-full flex-col pb-3">
    {workspace === 'shop' ? <>
      ...
      <Link href="/scan" className="mx-1 flex min-h-11 items-center gap-3 rounded-xl bg-[var(--admin-primary)] px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:opacity-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-primary)] focus-visible:ring-offset-2"><IconScan size={20} /><span className="flex-1">Service repas</span><span className="text-[10px] font-medium text-white/75">SCAN</span></Link>
    </>}
    ...
    {isPlatformOwner && <><p className={groupLabel}>Plateforme</p><Link href="/admin/team" className={linkClass(pathname === '/admin/team')}><IconUsers size={20} />Équipe</Link><Link href="/admin/platform/notifications" className={linkClass(pathname.startsWith('/admin/platform/notifications'))}><IconBell size={20} />Tests notifications</Link></>}
  </nav>;
}
