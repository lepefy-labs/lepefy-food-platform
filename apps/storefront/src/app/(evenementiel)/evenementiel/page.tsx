        ...
        <div className="relative z-[2] mx-auto flex min-h-[520px] max-w-[1180px] items-center px-4 py-14 sm:px-6 lg:min-h-[570px]">
          <div className="max-w-[620px] text-white">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href="#services" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/35 bg-white/10 px-5 text-sm font-semibold backdrop-blur-sm hover:bg-white/15">
                Voir nos services
              </a>
            </div>
          </div>
            ...
            {featuredEvent ? (
              <div className="space-y-5">
                <Link href={`/evenementiel/evenements/${featuredEvent.slug}`} className="group grid overflow-hidden rounded-[28px] border border-black/[0.06] bg-white shadow-[0_18px_45px_rgba(50,37,20,.08)] md:grid-cols-[1.15fr_.85fr]">
                  <EventImageFader
                    images={photosByEvent.get(featuredEvent.id) ?? (featuredEvent.banner_image_url ? [featuredEvent.banner_image_url] : [])}
                  ...
                  <div className="flex flex-col justify-between p-5 sm:p-7 md:p-8">
                    <div>
                      ...
                      <h3 className="mt-2 font-display text-3xl font-semibold leading-tight sm:text-4xl">{featuredEvent.title}</h3>
                      {featuredEvent.subtitle && <p className="mt-2 text-sm text-gray-600">{featuredEvent.subtitle}</p>}
                      <div className="mt-6 grid gap-3 text-sm text-gray-650 sm:grid-cols-2">
        ...
        {tenant.services_enabled && services.length > 0 && (
          <section id="services" className="scroll-mt-24 pb-14 sm:pb-18">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {traiteur && (
                <Link id="traiteur" href={`/evenementiel/services/${traiteur.slug}`} className="group relative min-h-[310px] overflow-hidden rounded-[28px] bg-[var(--color-primary-dark)] text-white shadow-lg">
              ...
              {location && (
                <Link id="location" href={`/evenementiel/services/${location.slug}`} className="group relative min-h-[310px] overflow-hidden rounded-[28px] bg-[#e9dcc1] text-[#1f281f] shadow-lg">
