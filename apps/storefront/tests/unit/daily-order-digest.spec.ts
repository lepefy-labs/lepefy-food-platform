...
test('urgent cards have a prominent red CTA while other cards keep simple links',()=>{
  const base={reference:'#EXAMPLE',customer:'Marie',reason:'Suivi à vérifier',
    action:'Vérifier la fiche',createdAt:now.toISOString()};
  const items=[
    {...base,key:'order:urgent',priority:'urgent' as const,url:'https://example.com/admin/orders/urgent'},
    {...base,key:'order:today',priority:'today' as const,url:'https://example.com/admin/orders/today'},
    {...base,key:'order:monitor',priority:'monitor' as const,url:'https://example.com/admin/orders/monitor'},
  ];
  const html=renderDigestHtml('Tenant','2026-09-26',items,[],'https://example.com/admin',null);
  expect(html).toContain('background-color:#b91c1c');
  expect(html).toContain('Traiter cette commande →');
  expect(html).toContain('href="https://example.com/admin/orders/urgent"');
  expect((html.match(/Traiter cette commande/g)??[])).toHaveLength(1);
  expect((html.match(/Voir la fiche/g)??[])).toHaveLength(2);
  expect(html).toContain('href="https://example.com/admin/orders/today"');
  expect(html).toContain('href="https://example.com/admin/orders/monitor"');
});
