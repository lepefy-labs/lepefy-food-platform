...
| Integrare design "Mon compte" (header brand, loyalty card reale, indirizzi, CTA) | Robertin | ⚠️ Prompt Claude Code generato (31/07), **esecuzione non confermata in chat** — vedi §37 |
| Integrare redesign home page (hero carousel, category blocks scroll, "Suggestions pour vous", FAB chat, admin hero slides) | Robertin | ✅ FATTO — eseguito, seguito da un ciclo di fix (autoscroll mobile, layout responsive) — **nessun `pnpm typecheck` esplicitamente riportato in chat** per questo ciclo, vedi §37 |
| Link di pagamento diretto su digital card (`extra.link`, nessuna migration) | Robertin | ⚠️ Prompt generato e approvato su mockup (2/08), **esecuzione non confermata in chat** — vedi §37 |
| Spostare `ProductSpecs` subito dopo il prezzo in PDP + trattamento colore vivace | Robertin | ⚠️ Prompt generato e approvato su mockup (2/08), **esecuzione non confermata in chat** — vedi §37 |
| Sistema Ambassadeur (commissioni euro al primo ordine referral, sconto opzionale, ruolo `tenant_cashier`) | Robertin | ⚠️ **Non avviato** — solo prompt Claude Code pronto in output (`ClaudeCode_Prompt_AmbassadorCommissionSystem.md`), non ancora inviato/eseguito, vedi §37 |
| Carta fedeltà con barcode EAN-13 proprio (namespace `21`) per accumulo punti in negozio | Robertin | ⚠️ **Non avviato** — solo design/decisioni chiuse in chat, prompt non confermato generato/eseguito, vedi §37 |
| Aprire account Google Play Console e avviare verifica identità | Robertin | ⚠️ DA FARE — **personale** scelto (non Organization), gate 12 tester/14gg nel percorso critico, vedi §37 |
| Migration `tenants.android_package_name`/`android_sha256_fingerprint` + route dinamica `/.well-known/assetlinks.json` | Robertin | ⚠️ DA FARE — solo pianificato, nessun prompt ancora generato, vedi §37 |
...
| Sistema Ambassadeur (commissioni euro primo ordine, sconto opzionale, ruolo `tenant_cashier`) | Business/Growth | P1 | Progettato e discusso (analisi sostenibilità inclusa), prompt Claude Code pronto — **non eseguito**, vedi §37 |
| Carta fedeltà EAN-13 propria (namespace `21`) + widget dashboard in stile tessera fisica | Business/Growth | P1 | Decisioni chiuse in chat — **non eseguito**, vedi §37 |
| Pubblicazione Play Store (TWA, Bubblewrap) | Growth | P1 | In pianificazione — nessun codice scritto, account Play Console personale scelto, gate 12 tester/14gg da avviare, vedi §37 (sostituisce/precisa la riga generica "Google Play Store via TWA/PWABuilder" già presente sopra) |
...
*Lepefy Labs — Lepefy Food Platform — Context document v3.19 — 3 Agosto 2026 (base: v3.18; aggiornamento da ricerca chat, non da verifica filesystem/git — cinque cicli 1–3 agosto: integrazione "Mon compte" [non eseguita], redesign home page [eseguito con fix], digital card + PDP [prompt generati], sistemi Ambassadeur/carta fedeltà EAN-13 [progettati, non eseguiti], pianificazione Play Store [solo analisi] — vedi §37 per il dettaglio completo)*
