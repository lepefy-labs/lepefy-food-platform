...
> Aggiornato: 20 Agosto 2026 (v3.59) — **nuovo Cart Drawer/Mini Cart**, blocco UX/UI esplicitamente successivo al carrello cross-device di v3.58: il click sul carrello dal Header desktop apre ora un pannello laterale (side drawer, ~420px) invece di navigare direttamente su `/cart` — overlay + focus trap + ESC + clic esterno, senza alcuna libreria Dialog/Sheet/Toast (nessuna nel repo, stesso pattern hand-rolled già usato da `ChatWidget`). Nuovi componenti in `components/cart/`: `CartDrawer`, `CartDrawerHeader` (conteggio "1 produit"/"N produits"), `CartItem`+`CartQuantityControl` (stepper dedicato, distinto dalla `QuantitySelector` della PDP), `CartDrawerFooter` (sous-total, CTA "Voir mon panier"→`/cart`, bannière sync discreta solo per offline/errore definitivo — mai per il `conflict` transitorio già auto-risolto dal sync engine v3.58), `CartDrawerEmpty` (stesso testo già validato su `/cart`), `CartUndoToast` (Undo realmente funzionante via le sole API pubbliche `addItem`/`removeItem` del cart store, nessuna nuova race condition). **Perimetro deliberatamente ridotto**: `/cart` (pagina completa) e `BottomNav` mobile non toccati — restano rispettivamente la "full cart page" a valle del drawer e il territorio del futuro blocco "Mobile Cart UX", entrambi esplicitamente fuori scope per questo ciclo; il drawer è comunque già responsive (bottom sheet <640px, side drawer da 768px), pronto per quel blocco senza riscrittura. Zero accesso diretto a `cartSyncEngine`/`cartApi`: solo le API pubbliche già esistenti del cart store, sync interamente invariata. **17 nuovi test unitari puri** (84/84 totali verdi, i 67 del ciclo v3.58 invariati) + `pnpm typecheck` pulito. **Anteprima browser reale impossibile** in questa sessione (nessuna credenziale Supabase — confermato concretamente con `pnpm dev` → `500 supabaseUrl is required`, non solo assunto): sostituita da una verifica statica con **CSS Tailwind realmente compilato** dal CLI del progetto su 7 breakpoint (360→1440px) via Playwright/Chromium — zero overflow orizzontale, bottoni quantità 44×44px esatti, `line-clamp-2` verificato via misure reali. Questa verifica ha **scoperto un bug reale**: il toast "Annuler" si sovrapponeva al bottone "Continuer mes achats" (misurato con `getBoundingClientRect()`, non a occhio) — corretto spostando il toast dal flusso overlay a quello normale del documento, ri-verificato `overlap: false`. Dettaglio completo, incluso l'elenco dei limiti dichiarati, in §77 (nuova sezione). Revisione precedente (v3.58) sotto.
...
│       │   │   ├── cart/                     # Nuovo v3.58 — sync engine cross-device, vedi §76
│       │   │   │   ├── cartSyncEngine.ts      # Flush debouncato, retry/backoff, offline, reconcile 409, merge login
│       │   │   │   ├── cartApi.ts             # Unico punto di contatto con /api/customers/me/cart
│       │   │   │   ├── cartTypes.ts, cartErrors.ts, cartLog.ts
│       │   │   │   ├── cartQueue.ts, reconcile.ts, mergeCarts.ts  # Funzioni pure, testate senza I/O
│       │   │   │   └── cartRequest.ts         # Validazione payload lato server (route + test)
...
| `carts` (`068_carts.sql`, nuova v3.49; **estesa in `070_cart_versioning.sql`, v3.58**) | Carrello server-side per clienti autenticati (continuità cross-device) — `items` jsonb `[{product_id, quantity}]`, mai prezzo/nome, unique su `(tenant_id, customer_id)`. **`070`** aggiunge `version` (bigint, default `1`, optimistic concurrency control) + `applied_mutation_ids` (ring buffer ultimi 100, idempotenza) + funzione atomica `apply_cart_mutations()` (`SELECT ... FOR UPDATE`, mutation tipizzate `add`/`set_quantity`/`remove`/`clear`/`replace`) — sostituisce il PUT full-state last-write-wins di `068`. Retrocompatibile, nessun dato riscritto. Vedi §76 |
...
| `docs/CART_SYNC.md` | **Nuovo v3.58, versionato nel repo** (non un allegato di sessione) — architettura sync cross-device del carrello, formato API `GET`/`POST /api/customers/me/cart`, strategia di risoluzione conflitti (add concorrenti, quantity conflict, merge login), procedura di verifica manuale a 13 passi. Referenziato anche da `CLAUDE.md`. Vedi §76 |
...
`pnpm typecheck` verde, `pnpm lint` mai eseguito. Nessun commit pushato — zip fornito su richiesta esplicita. Non testato end-to-end (nessun ambiente Supabase disponibile in sessione) — i 6 test manuali del prompt (toggle OTP/password, invito senza email Supabase, ricezione OTP standard, login completo, anti-enumerazione, admin disattivato) restano da eseguire dopo il deploy.

**✅ Aggiornamento manuale (19/08, post-deploy) — webhook n8n `/webhook/admin-invited` configurato e testato funzionante.** Non verificabile da Claude Code (workflow n8n, fuori dal repo): Webhook trigger (path `admin-invited`, respond immediately) → Send Email (SMTP Brevo, credenziale già esistente riusata). **Bug scoperto e corretto in configurazione, non nel codice**: il nodo Webhook di n8n incapsula il payload POST dentro `body` — tutti i riferimenti `{{ $json.* }}` nel nodo Send Email dovevano diventare `{{ $json.body.* }}` (`email`, `role`, `tenantName`, `invitedByEmail`, `loginUrl`). **Chiarito anche**: il campo `role` nel payload contiene già l'etichetta francese tradotta (`ROLE_LABELS[role]`, vedi `invite/route.ts` §68/§70), non l'enum grezzo — un nodo Edit Fields con ternary di traduzione ipotizzato inizialmente in n8n è stato rimosso perché ridondante. Template HTML email creato lato n8n (non nel repo). Test end-to-end email confermato da Robertin.
...
## 72. Changelog v3.54 (19 Agosto 2026) — Disattivazione Link su tutti i pagamenti + fix blocco "Traitement en cours…" — ⚠️ nessuno pushato

**Due bug distinti su `/card`, probabili su shop/event/rental (stesso composant partagé)** : (1) Link intercepte le `PaymentElement` avec l'écran "Accelerated Sign-up" et, en cas de trop de tentatives OTP, bloque le compte ("Troppi tentativi di accesso per questo account Link") ; (2) bug indépendant — quand la confirmation reste bloquée (pour Link, un redirect 3DS qui ne revient pas, ou toute autre cause), le bouton restait figé sur "Traitement en cours…" pour toujours, `isConfirming` n'ayant aucun chemin de reset. Explique aussi les PaymentIntent "incomplete" qui traînaient sur Stripe : tentatives abandonnées à cause du blocage UI, pas des échecs normaux. Décision confirmée : désactiver Link partout (paiement QR en boutique, aucune valeur ajoutée, risque de blocage) ; le fix du timeout reste une défense en profondeur indépendante de Link (couvre aussi une future réactivation accidentelle d'une méthode redirect-based).

### Step 0 — vérifié sur le filesystem

- Aucun des 5 points de création de PaymentIntent (`api/checkout/route.ts`, `api/checkout-sessions/[id]/create-intent/route.ts`, `api/card/quick-pay/route.ts`, `api/events/[id]/checkout/route.ts`, `api/rental/checkout/route.ts`) ne passait `payment_method_types` ni `automatic_payment_methods` — comportement dépendant entièrement des réglages Dashboard.
- **5ᵉ appelant de `StripePaymentStep` non mentionné dans le prompt initial** : `components/checkout-session/CheckoutSessionEditor.tsx` (module `shop`, flux de reprise d'une session de paiement en attente) — découvert par grep exhaustif sur `StripePaymentStep`, traité de façon identique aux 4 autres (voir Deviazioni).
- Recherche Stripe (web) : `excluded_payment_method_types` **ne peut explicitement pas exclure Apple Pay/Google Pay/Link** par transaction (limitation documentée côté Stripe) — seule une liste positive `payment_method_types` (sans `'link'`) ou une Payment Method Configuration Dashboard-side fonctionne. Aucun autre payment_method_type que `'card'` n'est référencé nulle part dans le code applicatif (grep `klarna|satispay|amazon_pay|sepa_debit|...` — zéro résultat pertinent) : Klarna/Satispay/Amazon Pay visibles dans le rapport de bug ne sont donc que le sous-menu propre à l'écran Link, jamais des payment_method_types réellement configurés dans cette intégration. `payment_method_types: ['card']` couvre aussi Apple Pay/Google Pay (wallets internes au type `'card'`, pas des types Stripe séparés) — aucune autre méthode active désactivée.
- `isConfirming` n'existe que dans `StripePaymentStep.tsx` (unique source de vérité pour les 4/5 flux) — confirmé aucun timeout/reset existant avant ce cycle.

### Cosa è stato costruito

- **`payment_method_types: ['card']` explicite** sur les 5 `stripe.paymentIntents.create(...)` — exclut Link sans dépendre du toggle Dashboard, aucune logique métier touchée (validation stock/prix, insert `checkout_sessions`/`tenant_card_payments`/metadata événement-rental strictement inchangés).
- **`paymentMethodTypes: ['card']` côté client**, sur les options du groupe `<Elements>` dans `StripePaymentStep.tsx` (pas sur `<PaymentElement>` — l'option n'existe qu'au niveau du groupe `Elements`, confirmé par les types `@stripe/stripe-js` locaux après un premier essai raté sur `<PaymentElement options>`). Nécessaire car `Elements` est monté en mode différé (`mode`/`amount`/`currency`, sans `clientSecret`) : sans ce champ, les méthodes affichées dépendent des réglages Dashboard, indépendamment du `payment_method_types` serveur — défense en profondeur pour couvrir le cas où le toggle Dashboard resterait actif.
- ~~**Timeout `CONFIRM_TIMEOUT_MS = 28_000`**~~ et ~~**prop `timeoutMessage`**~~ (+ `COPY.timeoutError` FR/IT dans `CardQuickPay.tsx`, littéraux FR dans les 4 autres appelants, log `confirm_timeout`) — **entièrement supprimés au cycle suivant, voir §73**. Ce mécanisme masquait le blocage derrière un message d'erreur générique au lieu d'en traiter la cause ; retiré sur décision de Robertin pour d'abord identifier la cause réelle. Aucune trace ne subsiste dans le code.

### Deviazioni

- **`CheckoutSessionEditor.tsx` traité alors que non cité dans le prompt** : c'est un 5ᵉ consommateur réel de `StripePaymentStep` (module `shop`, reprise de session en attente, §65 de ce document) — découverte toujours valable et réutilisée en §73, même si la modification concernée (`timeoutMessage`) a depuis été annulée.
- ~~**Aucun nouveau bouton "Réessayer"**~~ — sans objet depuis la suppression du timeout (§73).
- **`paymentMethodTypes` déplacé de `<PaymentElement>` vers `<Elements>`** : premier essai sur `<PaymentElement options>` rejeté par `tsc` (`StripePaymentElementOptions` ne l'expose pas) — corrigé après vérification des types `@stripe/stripe-js` installés, l'option existe uniquement au niveau du groupe `Elements`.
- **Nota permanente §66 (Accelerated Sign-up) non obsolète mais complétée** : cette note documentait que l'écran Link n'était pas désactivable par code — reste vrai pour le toggle Dashboard "Enable accelerated sign-up" lui-même, mais `payment_method_types: ['card']` empêche désormais Link d'être proposé comme méthode, donc l'écran ne devrait plus jamais s'afficher côté code, indépendamment de ce toggle. **Azione manuale Dashboard confermée nécessaire malgré tout** : Robertin doit désactiver Link dans Settings → Payment Methods → Link (toggle off) sur le compte Stripe actif — non automatisable par le code, défense en profondeur pour tout endpoint futur qui omettrait `payment_method_types`.
- Aucun mécanisme de nettoyage automatique des PaymentIntent/lignes pending abandonnés — explicitement exclu du prompt, non ajouté.
- Webhook Stripe et `payment_funnel_logs`/`logFunnelEvent` existants non touchés, comme demandé.

### Stato

`pnpm typecheck` verde (après `pnpm install`, `node_modules` absent au début du cycle). Nessun commit pushato — zip fornito su richiesta esplicita (non push). Non testato end-to-end (nessun ambiente Stripe/browser disponibile in sessione).

File toccati (**après annulation du timeout en §73**, il ne reste de ce cycle que) : `apps/storefront/src/components/payments/StripePaymentStep.tsx` (uniquement `paymentMethodTypes`) · `apps/storefront/src/app/api/checkout/route.ts` · `apps/storefront/src/app/api/checkout-sessions/[id]/create-intent/route.ts` · `apps/storefront/src/app/api/card/quick-pay/route.ts` · `apps/storefront/src/app/api/events/[id]/checkout/route.ts` · `apps/storefront/src/app/api/rental/checkout/route.ts` · `LEPEFY_PROJECT_CONTEXT.md`.
...
### Parte 1 — timeout supprimé

Supprimés de `StripePaymentStep.tsx` : `CONFIRM_TIMEOUT_MS`, `confirmTimeoutRef`, `clearConfirmTimeout()`, les 6 appels de nettoyage, le `setTimeout` d'ouverture, le log `confirm_timeout` et la prop obligatoire `timeoutMessage`. Supprimés des 5 appelants : la prop `timeoutMessage` et, dans `CardQuickPay.tsx`, le champ `timeoutError` de l'interface `QuickPayCopy` et ses deux traductions FR/IT. Le `useEffect` de montage est revenu à son `return registerAbandonmentListener(...)` direct (le wrapper de cleanup n'existait que pour le timeout). Vérification : `grep -rn "timeoutMessage\|timeoutError\|CONFIRM_TIMEOUT\|confirm_timeout\|clearConfirmTimeout\|confirmTimeoutRef"` → **zéro occurrence**. Les 4 fichiers appelants sont redevenus **strictement identiques à HEAD** (ils ont disparu du `git diff`) ; `StripePaymentStep.tsx` ne diffère plus de HEAD que par les 7 lignes de `paymentMethodTypes: ['card']`. Comportement restauré : `isConfirming` reste `true` tant que `confirmPayment()` ne se résout pas, sans aucun filet — temporaire et assumé.

### Parte 2 — analisi (nessuna modifica di codice)

**⚠️ Correction d'une erreur d'analyse commise en cours de cycle** : une première version de cette section affirmait que §72 n'était pas déployé, sur la foi du `HEAD` local. C'était **faux** — le `HEAD` local était périmé et `origin/main` avait été force-pushé entre-temps. Après `git fetch`, `origin/main` (commit `f953166`, 20/08 00:39 CEST) contient **bien** l'intégralité de §72 : `payment_method_types: ['card']` sur les 5 routes serveur, `paymentMethodTypes: ['card']` sur `<Elements>`, **et** le timeout de 28s. Toujours vérifier `origin/main` après `git fetch`, jamais le `HEAD` local, avant de conclure sur l'état déployé.

**Fait décisif qui en découle** : les symptômes ont été observés **avec les deux couches de protection actives**. Robertin rapporte avoir vu le blocage masqué par le timeout de 28s — or ce timeout n'existe que dans `f953166`, donc son test portait bien sur le code post-§72. Conclusion : **`paymentMethodTypes: ['card']` n'empêche pas l'UI d'inscription Link inline de s'afficher** — ce qui recoupe exactement la note permanente de §66 (l'Accelerated Sign-up est un réglage Dashboard, hors de portée du code). Restreindre les méthodes de paiement et supprimer l'UI Link sont deux choses différentes.

1. **Chemin de l'email** (`StripePaymentStep.tsx:117`) : `defaultValues: customerEmail ? { billingDetails: { email: customerEmail } } : undefined`. Quand `/card` laisse l'email vide, `CardQuickPay.tsx:134` passe `customerEmail={customerEmail.trim() || undefined}` → `defaultValues` vaut `undefined`, et le Payment Element démarre sans email connue. `fields` ne déclare que `billingDetails.address: 'never'` (ligne 116) : `email` reste donc en `'auto'`, et pour une carte l'email n'étant pas requise, aucun champ email n'est rendu par l'Element — Link occupe cet espace avec sa propre UI inline ("Checkout sicuro e veloce con Link"), ce qui correspond exactement au bandeau observé.
2. **Cohérence avec le comportement documenté** : oui. La doc Stripe `confirmPayment` indique que *lorsque le paiement requiert une étape supplémentaire (authentification), la promesse ne se résout pas tant que cette étape n'est pas terminée ou expirée*. L'authentification Link **est** cette étape : tant que l'utilisateur reste coincé dans l'OTP Link (ou est bloqué par "trop de tentatives"), la promesse reste légitimement pendante — donc `isConfirming` reste `true`. Ce n'est pas un bug Stripe ni un bug de notre `await` : c'est le contrat de l'API. La doc Link précise par ailleurs que préremplir l'email *initie* l'authentification Link (elle ne la désactive pas) — d'où le scénario email-remplie qui « semble » fonctionner : l'étape se résout plus vite, elle n'est pas supprimée.
3. **Cause secondaire indépendante, trouvée dans notre code** : `handleConfirm` (lignes 44-104) n'a **aucun `try/catch`**, et `onClick={handleConfirm}` (ligne 122) ignore la promesse retournée. Toute exception *levée* (et non retournée dans `{ error }`) sort de la fonction sans jamais atteindre un `setIsConfirming(false)` → bouton figé définitivement, sans message. Ce n'est pas théorique : aucune des 3 implémentations de `createIntent` (`CardQuickPay.tsx`, `EventCheckoutClient.tsx`, `RentalCheckoutClient.tsx`) n'entoure son `fetch`/`res.json()` d'un `try/catch` — une coupure réseau ou une réponse non-JSON lève et fige le bouton, **sur les 4 modules, indépendamment de Link**. À noter aussi : `fields.billingDetails.address: 'never'` (ligne 116) sans `payment_method_data.billing_details.address` dans `confirmParams` (ligne 80) est un motif que Stripe traite en `IntegrationError` levée — piste à vérifier, non confirmée faute d'accès à `docs.stripe.com` (bloqué par le proxy réseau de la session).
4. **Périmètre** : l'email est **obligatoire et validée avant l'étape de paiement** dans les 3 autres modules — shop (`CheckoutForm.tsx:33`, zod `.email()`), event (`EventCheckoutClient.tsx:178-184`), rental (`RentalCheckoutClient.tsx:88-89`) ; `CheckoutSessionEditor.tsx:401` passe `data.email` (typé `string` non optionnel, ligne 50). Le scénario « email vide » est donc **exclusif à `/card`**. En revanche, la cause n°3 (absence de `try/catch`) touche **les 5 flux sans exception**.

### Parte 2bis — bisection rétrospective du module commun (demandée par Robertin)

Historique complet de `components/payments/StripePaymentStep.tsx` sur `origin/main` — 4 versions :

| Commit | Date | Ce qui change dans le module commun |
|---|---|---|
| `b3fc525` | 18/08 | §63 — création du composant partagé. `<PaymentElement />` **nu, sans aucune option** ; `<Elements options={{ mode, amount, currency }}>` seulement. |
| `917b3db` | 19/08 | **§66 — le seul commit qui touche les options du Payment Element.** Ajoute d'un bloc : `layout: 'accordion'`, `fields: { billingDetails: { address: 'never' } }`, `defaultValues: customerEmail ? {...} : undefined`, `appearance`. Côté `CardQuickPay.tsx`, une seule ligne ajoutée : `customerEmail={customerEmail.trim() \|\| undefined}`. |
| `5346f27` | 19/08 | Agent e2e — `isTest?` + `getStripeForModule(module, isTest)`. **Aucun rapport** avec le Payment Element. |
| `f953166` | 20/08 | §72 — timeout 28s + `paymentMethodTypes: ['card']`. |

**La fenêtre de régression est donc `b3fc525 → 917b3db` (§66)** : c'est le seul changement qui modifie ce que le Payment Element rend à l'écran. Avant §66, l'Element était nu ; après, il porte 4 options d'un coup.

**L'hypothèse de Robertin (le préremplissage de l'email) est écartée par le code lui-même** : `defaultValues: customerEmail ? { billingDetails: { email: customerEmail } } : undefined` (ligne 159 de `f953166`). Dans le scénario qui échoue — `/card`, email vide — `CardQuickPay.tsx` passe `customerEmail.trim() || undefined`, donc `customerEmail` est `undefined`, donc **l'expression entière vaut `undefined` et l'autofill est littéralement inerte au moment précis où le bug se reproduit**. Une ligne qui ne s'exécute pas ne peut pas être la cause. Le préremplissage est en revanche ce qui *masque* le problème dans les 3 autres modules (email obligatoire ⇒ `defaultValues` toujours fourni), ce qui explique la corrélation observée sans en être la cause.
...
- **Erreur d'analyse corrigée en cours de cycle** : la conclusion initiale « §72 non déployé » était fausse (`HEAD` local périmé, `origin/main` force-pushé). Corrigée après `git fetch` — voir l'encadré en tête de Parte 2. Signalée plutôt que réécrite silencieusement.
- **`LEPEFY_PROJECT_CONTEXT.md` modifié alors que le prompt ne le demandait pas** : §72 décrivait un timeout qui n'existe plus dans le code — laisser cette section intacte aurait rendu le document faux. Passages concernés barrés et renvoyés vers §73, plutôt que supprimés (traçabilité de la décision). Signalé plutôt qu'appliqué silencieusement.
- Aucune correction appliquée pour la cause n°3 (absence de `try/catch`) : le prompt interdit explicitement toute implémentation dans ce cycle. Reportée comme option (c) ci-dessus.

### Stato

`pnpm typecheck` verde. Nessun commit pushato — zip fornito su richiesta esplicita (non push).
...
## 77. Changelog v3.59 (20 Agosto 2026) — Cart Drawer / Mini Cart: nuovo punto di accesso rapido al carrello, sostituisce la navigazione diretta a `/cart` dal Header desktop — ⚠️ non ancora pushato

Ciclo mirato esclusivamente alla UX/UI del carrello (Cart Drawer, Mini Cart, Cart Item, Cart Summary, Empty Cart), blocco esplicitamente successivo al carrello cross-device di v3.58 (§76). Nessuna modifica a Stripe/checkout/payment logic/authentication/product catalog business logic/shipping business logic/database cart synchronization/API sync logic, come da vincolo esplicito del prompt.

### Step 0 — audit prima del codice

Confermato: nessun `CartDrawer`/`MiniCart`/`Dialog`/`Sheet`/`Toast` esisteva in tutto lo storefront (grep mirato) — il click sul carrello era sempre navigazione diretta a `/cart` (`CartClient.tsx`, pagina completa). Nessuna libreria di overlay nel repo (né storefront né admin): gli overlay esistenti (`ChatWidget`, `BulkTrackingModal` admin) sono `fixed` fatti a mano, senza portale — convenzione riusata qui, nessuna libreria introdotta. Confermato anche: `totalItems()` del cart store somma già le quantità (non le righe), quindi il badge carrello era già semanticamente corretto — nessuna correzione necessaria lì.

**Scoperta rilevante sul free-shipping progress (§12 del prompt):** il repo non ha una soglia di spedizione gratuita globale — solo `shipping_country_rules.free_shipping_above`, risolta server-side per paese/CAP (`resolveCountryRule.ts`), quindi dipendente da un indirizzo che il drawer non ha. Introdurla avrebbe richiesto una nuova chiamata di rete + business logic di spedizione, esplicitamente fuori perimetro. `FreeShippingProgress.tsx` è stato quindi costruito come richiesto dal prompt per questo caso — puro, riceve `threshold`/`subtotal` come prop, non renderizza nulla se `threshold` è `null` — e il footer lo monta passando `threshold={null}`: pronto per una soglia reale futura, senza inventare dati.

### Componenti nuovi (`components/cart/`)

`CartDrawer.tsx` (orchestratore: overlay + panel, focus trap, ESC, scroll lock, click esterno) · `CartDrawerHeader.tsx` (titolo + conteggio dinamico "1 produit"/"N produits") · `CartItem.tsx` + `CartQuantityControl.tsx` (stepper dedicato, distinto da `QuantitySelector.tsx` della PDP che accetta input libero — caso d'uso diverso, non un duplicato) · `CartDrawerFooter.tsx` (sous-total, CTA "Voir mon panier"/"Continuer mes achats", bannière sync) · `CartDrawerEmpty.tsx` (stesso testo già validato su `/cart`, riuso letterale) · `CartUndoToast.tsx` · `FreeShippingProgress.tsx`. Più `stores/cartUiStore.ts` (Zustand, `isDrawerOpen` **non persistito** — stato UI éphémère separato da `cartStore.ts`, mai una seconda source of truth per i dati carrello, §23/§24 del prompt rispettati: selettori dedicati in `lib/cart/cartSelectors.ts` che leggono un solo campo alla volta, `lib/cart/cartItemState.ts` per lo stato visivo per riga, `hooks/useFocusTrap.ts` (~40 righe, nessuna libreria a11y nel repo).

### Perimetro deliberatamente ridotto (scelte di scope, non omissioni)

- **`CartClient.tsx`/pagina `/cart` non toccata** — è esplicitamente la "FULL CART PAGE" a valle del drawer nel diagramma del prompt (§11), non nell'elenco dei componenti in scope. `CartItem` è nuovo, costruito per il drawer; nessun refactor della pagina esistente per non introdurre rischio di regressione su un flusso già stabile.
- **`BottomNav.tsx` (mobile) non toccata** — il tab "Panier" continua a navigare direttamente su `/cart`. Il prompt vieta esplicitamente di "implementare ancora tutta la mobile cart UX" in questo ciclo; il componente `CartDrawer` è comunque già responsive (bottom sheet sotto `sm:`, side drawer da `sm:` in su), pronto per un futuro blocco "Mobile Cart UX" senza riscrittura.
- **`ProductCard`/`SemanticProductCard`/`ProductDetail` non toccati** — hanno già un feedback inline (bottone che diventa "✓" per 1,5–2s) che soddisfa il requisito "discreto, non bloccante" del prompt; il drawer non si apre automaticamente ad ogni aggiunta, come richiesto esplicitamente.

### Sync integration

`CartDrawer` non possiede alcuna copia del carrello: legge `items`/`syncStatus`/`unavailableProductIds`/`pendingMutations` da `cartStore` via selettori dedicati, e chiama solo le API pubbliche già esistenti — `updateQuantity()` per lo stepper +/- (stessa API, stessa semantica assoluta già usata da `CartClient.tsx`, per coerenza tra drawer e pagina), `removeItem()`/`addItem()` per il retiro/l'annullamento. **Zero accesso diretto a `cartSyncEngine`/`cartApi`/`fetch()`** — la sync resta interamente sotto il motore v3.58, invariato. La bannière di sync (§10 del prompt) mostra un messaggio solo per `offline` ed `error` **definitivo**: lo stato `conflict` è sempre riconciliato e ritentato automaticamente da `cartSyncEngine` (v3.58) e non diventa mai visibile all'utente finché non degenera in `error` dopo aver esaurito i tentativi — mappatura dichiarata esplicitamente nel codice.

### Undo funzionante (non scartato)

Il prompt permetteva di non implementare l'Undo se troppo complesso. Implementato comunque, a basso rischio: `handleRemove` cattura uno snapshot `{product, quantity}` **prima** di chiamare `removeItem()`, mostra il toast per 5s, e "Annuler" richiama `addItem(product, quantity)` — le stesse due API pubbliche già usate ovunque nell'app, nessuna chiamata diretta al sync engine, nessuna race condition nuova (la mutation `add` risultante entra nella stessa coda debounced di sempre).

### Accessibilità

`role="dialog"` + `aria-modal="true"` + `aria-labelledby`; focus trap con Tab/Shift+Tab ciclici e ripristino del focus al trigger alla chiusura (`useFocusTrap.ts`); chiusura su Échap, clic sull'overlay, bottone di chiusura (44×44px, `aria-label="Fermer le panier"`); tutti i bottoni +/-/retirer hanno `aria-label` dinamico (es. "Réduire la quantité de {nom}"); `aria-live="polite"` su conteggio riga, totale riga, bannière sync e toast; `:focus-visible` già globale (nessun lavoro aggiuntivo necessario, token di piattaforma esistente); `motion-reduce:transition-none`/`motion-reduce:animate-none` su tutte le transizioni. **Limite noto dichiarato**: nessun `inert`/`aria-hidden` applicato al contenuto di pagina dietro l'overlay (stessa scelta di `ChatWidget`/`BulkTrackingModal`, nessun altro overlay del repo lo fa).

### Responsive — verificato con misure reali, non solo classi Tailwind lette

Bottom sheet (`inset-x-0 bottom-0 max-h-[85vh] rounded-t-2xl`) sotto `sm:`, side drawer (`sm:right-0 sm:h-full sm:max-w-[420px]`) da `sm:` in su, stessa struttura DOM per entrambi (solo classi responsive) — nessuna riscrittura necessaria per un futuro affinamento mobile.

### Test

**17 nuovi test unitari puri** (stesso runner Playwright-come-test-runner del ciclo v3.58, zero nuova dipendenza): 7 su `cartSelectors.ts` (conteggio unità non righe, subtotale, dedup mutation pendenti), 7 su `cartItemState.ts` (priorità unavailable > out_of_stock > pending > normal, tutte le combinazioni), 3 su `formatProductCount.ts`. **84/84 test totali passati** (67 preesistenti del ciclo v3.58, invariati, non riscritti — vietato esplicitamente dal prompt). I test di interazione richiesti dal prompt (renders/opens/closes/ESC/outside-click/CTA-navigates a livello DOM) **non sono stati scritti come suite eseguibile**: il repo non ha `@testing-library/react`/jsdom né un harness di component-test, e introdurne uno sarebbe una nuova dipendenza non necessaria per un solo ciclo — vedi "Verifica" sotto per come questi comportamenti sono stati comunque verificati con evidenza reale.

### Validazione — risultati reali

- `pnpm typecheck` — **pulito**, eseguito due volte (prima e dopo il fix del bug scoperto in verifica, sotto), unico comando la cui esecuzione era richiesta esplicitamente per questo ciclo.
- **Anteprima browser reale del sito**: **non possibile in questa sessione** — nessuna variabile d'ambiente Supabase/Stripe presente nel sandbox, confermato concretamente (non solo assunto): `pnpm dev` avviato, richiesta a `/` → `500, Error: supabaseUrl is required` (stesso limite già documentato in §75 per lo stesso repo). Fallback: **verifica visiva reale** via una pagina HTML statica che riproduce esattamente il markup e le classi Tailwind dei nuovi componenti, con il **CSS realmente compilato dal CLI Tailwind del progetto** (stesso `tailwind.config.ts`, stessi token `:root` di `globals.css`) invece di CSS indovinato, aperta con Chromium via Playwright a 360/390/430/768/1024/1280/1440px:
  - nessun overflow orizzontale a nessuna larghezza (`scrollWidth === clientWidth` su tutte e 7);
  - bottone quantità misurato **44×44px** esatti;
  - nome prodotto lungo: clampato a **2 righe** (`line-clamp-2`, verificato via altezza/line-height calcolati, non a occhio), nessuno sforamento di layout;
  - CTA principale sempre interamente visibile nel viewport a tutte le larghezze;
  - bottom sheet reale sotto 640px (pannello ancorato in basso, altezza contenuto-dipendente entro l'85vh), side drawer reale da 768px in su (pannello a destra, larghezza 420px fissa).
  - **Bug reale trovato e corretto grazie a questa verifica**: il toast di annullamento (`position: absolute; bottom: 3`) si sovrapponeva al bottone "Continuer mes achats" del footer — misurato con `getBoundingClientRect()` (non un'impressione visiva), non un'ipotesi. Corretto spostando il toast nel flusso normale del documento (tra la lista articoli e il footer) invece che in overlay assoluto: nessuna sovrapposizione possibile qualunque sia il contenuto del footer (bannière sync presente o meno). Ri-verificato dopo il fix: `overlap: false`.
- File temporanei della verifica (HTML statico, config Tailwind di prova, screenshot) creati solo nello scratchpad di sessione, **mai committati** — confermato `git status` pulito su questo prima del commit.
- Effetto collaterale scoperto e annullato: l'avvio di `pnpm dev` per il test sopra ha fatto riscrivere automaticamente `tsconfig.json` da Next.js (riformattazione + due opzioni aggiunte) e generato `next-env.d.ts` — nessuno dei due era un cambiamento intenzionale di questo ciclo: `tsconfig.json` ripristinato (`git checkout --`), `next-env.d.ts` rimosso prima del commit.

### Known limitations (dichiarate, per la fase successiva)

1. Nessun test DOM/interazione eseguibile (renders/opens/ESC/outside-click) — richiederebbe `@testing-library/react`+jsdom, non introdotto in questo ciclo.
2. Mobile: `BottomNav` continua a navigare su `/cart` a pagina piena, il bottom-sheet del drawer è costruito ma non ancora collegato a un trigger mobile — riservato al blocco "Mobile Cart UX" esplicitamente menzionato dal prompt come fase successiva.
3. Nessuna animazione di uscita per-riga alla rimozione (stato "REMOVING" del prompt) — la rimozione è immediata (local-first, come richiesto), senza fade-out della riga: scelta deliberata per semplicità, nessun timer/lifecycle aggiuntivo introdotto.
4. Nessun `inert`/`aria-hidden` sul contenuto dietro l'overlay (vedi Accessibilità sopra).
5. `FreeShippingProgress` costruito ma non alimentato con una soglia reale (vedi Step 0) — in attesa di una fonte dati appropriata, indipendente da questo ciclo.

### Stato

Consegnato via zip su richiesta esplicita ("non fare push"), commit locale sul branch di lavoro `claude/new-session-2evnvx`.

File aggiunti: `apps/storefront/src/components/cart/{CartDrawer,CartDrawerHeader,CartDrawerFooter,CartDrawerEmpty,CartItem,CartQuantityControl,CartUndoToast,FreeShippingProgress}.tsx` · `apps/storefront/src/stores/cartUiStore.ts` · `apps/storefront/src/lib/cart/{cartSelectors,cartItemState,formatProductCount}.ts` · `apps/storefront/src/hooks/useFocusTrap.ts` · `apps/storefront/tests/unit/{cartSelectors,cartItemState,formatProductCount}.spec.ts`.
File modificati: `apps/storefront/src/components/layout/Header.tsx` (link "Panier" desktop apre il drawer, clic modificato/rotellina restano navigazione normale) · `apps/storefront/src/app/(shop)/layout.tsx` (mount di `<CartDrawer />`, stesso pattern di `<ChatWidget />`).
