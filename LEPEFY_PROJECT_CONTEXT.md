...
**Fait décisif établi en premier** : le fix `payment_method_types: ['card']` de §72 **n'est pas déployé** (`git status` : les 6 fichiers sont non commités, aucun push effectué depuis §72). Les symptômes observés par Robertin proviennent donc du code **sans** ce fix, avec Link actif côté Dashboard — les deux couches de protection manquaient au moment du test.
...
**Conclusion** : désactiver Link côté Dashboard **suffit à faire disparaître le symptôme observé** (plus d'étape Link ⇒ plus d'attente infinie sur cette étape), et le `payment_method_types: ['card']` de §72, une fois déployé, produit le même effet côté code. Mais cela **ne corrige pas** la cause n°3, qui reste un blocage définitif possible sur les 5 flux pour toute exception levée. Options présentées à Robertin (aucune implémentée) : (a) Dashboard seul ; (b) déployer §72 seul ; (c) ajouter un `try/catch` autour de `handleConfirm` avec `setIsConfirming(false)` dans le `finally` — correctif de robustesse ciblé, sans timeout ni nouvelle UI ; (d) rendre l'email obligatoire sur `/card` — écarté d'office comme régression produit (le caractère facultatif est un choix assumé du paiement par QR en boutique).
