...
*Lepefy Labs — Lepefy Food Platform — Context document v3.44 — 18 Agosto 2026 (base: v3.43; fix `return_url` mancante su `/card` con rischio WebView/QR documentato, nuova tabella condivisa `payment_funnel_logs` con 7 event_type, 4 moduli di pagamento strumentati con `reference_id` coerente per modulo (`quickPaymentId`/`event.id`/`service.id`/`sessionId`) — vedi §62 per il dettaglio completo)*
