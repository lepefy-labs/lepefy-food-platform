...
- **Timeout `CONFIRM_TIMEOUT_MS = 28_000`** (fenêtre 25-30s proposée par le prompt : large marge sur une confirmation carte normale ou un défi 3DS in-page, sans faire attendre inutilement un utilisateur réellement bloqué) démarré au clic "Payer", nettoyé (`clearTimeout`) à chaque sortie existante (`submitError`, erreur `createIntent`, erreur `confirmPayment`, `requires_action`, succès) et au démontage du composant. À l'expiration : `setIsConfirming(false)` (réactive le bouton "Payer", pas de nouveau bouton "Réessayer" nécessaire), `onError(timeoutMessage)`, plus un nouveau log `confirm_timeout` (proposé, pas dans les 4 points de log existants — signalé explicitement, aucun log existant modifié).
- **`timeoutMessage`** : nouvelle prop **obligatoire** de `StripePaymentStep`, fournie par chaque appelant dans son propre schéma de traduction — ajouté à `COPY.fr`/`COPY.it` (`timeoutError`) dans `CardQuickPay.tsx` ; littéral FR `"Le paiement met trop de temps à répondre. Veuillez réessayer."` dans `CheckoutForm.tsx`, `EventCheckoutClient.tsx`, `RentalCheckoutClient.tsx` et `CheckoutSessionEditor.tsx` (aucun COPY/Lang existant dans ces 4-là, cohérent avec leur `processingLabel` déjà en dur).

### Deviazioni

- **`CheckoutSessionEditor.tsx` traité alors que non cité dans le prompt** : c'est un 5ᵉ consommateur réel de `StripePaymentStep` (module `shop`, reprise de session en attente, §65 de ce document) — même correctif appliqué par cohérence (sinon `timeoutMessage` obligatoire aurait cassé le build), signalé explicitement plutôt qu'appliqué silencieusement.
- **Aucun nouveau bouton "Réessayer"** : le prompt suggérait un bouton dédié, mais réactiver `isConfirming=false` ré-affiche déjà le bouton "Payer" existant (comportement identique aux autres branches d'erreur) — pas de nouvel élément UI, jugé suffisant et cohérent avec l'existant.
...
`pnpm typecheck` verde (après `pnpm install`, `node_modules` absent au début du cycle). Nessun commit pushato — zip fornito su richiesta esplicita (non push). Non testato end-to-end (nessun ambiente Stripe/browser disponibile in sessione) — à vérifier après déploiement : absence de l'écran Link sur les 5 flux, et déclenchement réel du timeout (ex. en coupant le réseau après clic "Payer").
...
*Lepefy Labs — Lepefy Food Platform — Context document v3.54 — 19 Agosto 2026 (base: v3.53; disattivazione Link su tutti i pagamenti + fix blocco "Traitement en cours…" §72; nessuno pushato su main)*
