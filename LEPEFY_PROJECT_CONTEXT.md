...
>
> **Aggiornato:** 22 settembre 2026 — **v6.67 Current-State Snapshot**
>
...
- Shipping Intelligence : il mapping città→CAP delle campagne dipende da Nominatim + Zippopotam.us/GeoNames con fallback manuale; il mapping CAP→zona commerciale resta tenant-owned (`shipping_zones.postal_prefixes`) e non costituisce un mapping regionale ufficiale della piattaforma; `shipping_quote_observations.source = 'real_shipment'` non è mai popolato in V1 perché il costo finale reale di una spedizione non è catturato separatamente dal preventivo; `customer_pricing_strategy` (V1E, pricing cliente/checkout) non è implementato — Tarification/checkout restano interamente sul flusso `shipping_country_rules` esistente;
